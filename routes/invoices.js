...
router.post('/', fetchuser, async (req, res) => {
  try {
    const { customer, reference, total, currency,dueDate, items,  poNumber,
...
router.delete('/', fetchuser, async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'All invoices deleted', deletedCount: result.deletedCount });
  } catch (err) {
    console.error('Error deleting all invoices:', err);
    res.status(500).json({ message: 'Error deleting all invoices', error: err });
  }
});
...
router.put('/:id', fetchuser, async (req, res) => {
  try {
    ...
    const existingInvoice = await Invoice.findOne({ _id: invoiceId, user: req.user.id });
    if (!existingInvoice) {
